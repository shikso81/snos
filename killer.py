...
violations = {
    ...
    5: ['Виртуальный номер' ['Уважаемая служба поддержки, обращаем ваше внимание на абонента {username}, который использует номер который и близко к нему не относистся. Просим вас проверить и принять соответствующие меры.',
                            'Данная личность {username} использует виртуальный номер который даже близко не относится к нему, что недопустимо и требует вмешательства. Просим вас проверить и принять меры соответствующие меры']],
}
